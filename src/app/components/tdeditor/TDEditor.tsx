  ...
  return {
    upload: async () => {
      try {
        ...
        const response = await requestUploadFile<{ data: { url: string }[] }>('api/v1/attachments/public', formData);
...
export const TDEditor: FC<TDEditorProps> = memo(({ data, onChange, placeholder = '', readOnly = false, sanitizeOptions = sanitizeConfig }) => {
  ...
  return <CKEditor editor={ClassicEditor} config={config} data={sanitizedInitialData} onChange={handleChange} disabled={readOnly} />;
});
