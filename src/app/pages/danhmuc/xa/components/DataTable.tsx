...
const DataTable: React.FC<DataTableProps> = ({ searchData }) => {
  ...
  const { data,tinh,huyen, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useDataTable({ searchData });
  ...
  const columns: TableProps<IDiaBan>['columns'] = [
    ...
    },
    {
      ...
      key: 'maCha',
      render: (text, record) => {
        const huyens = huyen.find(prov => prov.ma === record.maCha);
        return (
          <div>{huyens ? huyens.ten : 'Không xác định'}</div>
        );
      },
    },
    {
      ...
      key: 'maCha',
      render: (text, record) => {
        // Tìm huyện dựa trên mã cha của record
        const huyenFound = huyen.find(h => h.ma === record.maCha);
    
        // Tìm tỉnh dựa trên maCha của huyện
        const tinhFound = huyenFound ? tinh.find(t => t.ma === huyenFound.maCha) : null;
    
        // Trả về tên tỉnh hoặc 'Không xác định'
        return (
          <div>
            {tinhFound ? tinhFound.ten : 'Không xác định'}
          </div>
        );
      },
    },
