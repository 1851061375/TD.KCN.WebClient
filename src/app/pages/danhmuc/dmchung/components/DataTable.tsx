...
import { IResult, IDanhMuc } from '@/models';
...
const DataTable: React.FC<DataTableProps> = ({ searchData }) => {
  ...
  const handleAction = useCallback(
    async (type: string, record: IDanhMuc): Promise<void> => {
      try {
        switch (type) {
          case 'chi-tiet':
          ...
          case 'delete':
            ...
            const response = await requestDELETE<IResult<boolean>>(`api/v1/danhmucs/${record.id}`);
  ...
  const columns: TableProps<IDanhMuc>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
