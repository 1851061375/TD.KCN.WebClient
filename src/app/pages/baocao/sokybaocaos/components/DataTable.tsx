...
import { ILoaiBaoCaoDto, IPaginationResponse, IResult } from '@/models';
...
const DataTable: React.FC<LoaiBaoCaosTableProps> = ({ searchData }) => {
  ...
  const [dataTable, setDataTable] = useState<ILoaiBaoCaoDto[]>([]);
  ...
  const fetchData = async (): Promise<void> => {
    try {
      setLoading(true);
      const response = await requestPOST<IPaginationResponse<ILoaiBaoCaoDto[]>>('api/v1/sokybaocaos/search', {
  ...
  const handleAction = useCallback(
    async (type: string, record: ILoaiBaoCaoDto): Promise<void> => {
      try {
        switch (type) {
          case 'chi-tiet':
          ...
          case 'delete':
            ...
            const response = await requestDELETE<IResult<boolean>>(`api/v1/sokybaocaos/${record.id}`);
  ...
  const columns: TableProps<ILoaiBaoCaoDto>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
