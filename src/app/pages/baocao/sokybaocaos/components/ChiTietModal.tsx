...
import { ISoKyBaoCaoDto, IResult, IPaginationResponse } from '@/models';
...
const ModalItem = props => {
  ...
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as ISoKyBaoCaoDto | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<ISoKyBaoCaoDto>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<ISoKyBaoCaoDto>>(`api/v1/sokybaocaos/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: ISoKyBaoCaoDto = {
        ...values,
        ...(id && { id }),
      };

      const response = id
        ? await requestPUT<IResult<string>>(`api/v1/sokybaocaos/${id}`, formData)
        : await requestPOST<IResult<string>>(`api/v1/sokybaocaos`, formData);
    ...
    >
      ...
      <Modal.Body>
        <Spin spinning={isLoading}>
          {!isLoading && (
            <Form<ISoKyBaoCaoDto> form={form} initialValues={{ suDung: true }} layout="vertical" autoComplete="off">
              <div className="row">
