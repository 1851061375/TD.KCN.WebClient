...
import { ILoaiKyDuLieuDto, IPaginationResponse, IResult } from '@/models';
...
const DataTable: React.FC<LoaiBaoCaosTableProps> = ({ searchData }) => {
  ...
  const [dataTable, setDataTable] = useState<ILoaiKyDuLieuDto[]>([]);
  ...
  const fetchData = async (): Promise<void> => {
    try {
      setLoading(true);
      const response = await requestPOST<IPaginationResponse<ILoaiKyDuLieuDto[]>>('api/v1/loaikydulieus/search', {
  ...
  const handleAction = useCallback(
    async (type: string, record: ILoaiKyDuLieuDto): Promise<void> => {
      try {
        switch (type) {
          case 'chi-tiet':
          ...
          case 'delete':
            ...
            const response = await requestDELETE<IResult<boolean>>(`api/v1/loaikydulieus/${record.id}`);
  ...
  const columns: TableProps<ILoaiKyDuLieuDto>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
