...
import { ILoaiKyDuLieuDto, IResult } from '@/models';
...
const ModalItem = props => {
  ...
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as ILoaiKyDuLieuDto | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<ILoaiKyDuLieuDto>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<ILoaiKyDuLieuDto>>(`api/v1/loaikydulieus/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: ILoaiKyDuLieuDto = {
        ...values,
        ...(id && { id }),
      };

      const response = id
        ? await requestPUT<IResult<string>>(`api/v1/loaikydulieus/${id}`, formData)
        : await requestPOST<IResult<string>>(`api/v1/loaikydulieus`, formData);
    ...
    >
      ...
      <Modal.Body>
        <Spin spinning={isLoading}>
          {!isLoading && (
            <Form<ILoaiKyDuLieuDto> form={form} initialValues={{ suDung: true }} layout="vertical" autoComplete="off">
              <div className="row">
