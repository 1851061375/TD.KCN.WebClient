...
interface LoaiTongHopsTableProps {
  searchData?: SearchData;
}

const DataTable: React.FC<LoaiTongHopsTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const random = useSelector((state: RootState) => state.global.random);

  const [dataTable, setDataTable] = useState<ILoaiTongHopDto[]>([]);
  ...
  const handleAction = useCallback(
    async (type: string, record: ILoaiTongHopDto): Promise<void> => {
      try {
        switch (type) {
          case 'chi-tiet':
  ...
  const columns: TableProps<ILoaiTongHopDto>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
