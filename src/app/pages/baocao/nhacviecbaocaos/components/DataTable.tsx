...
const DataTable: React.FC<LoaiBaoCaosTableProps> = ({ searchData }) => {
  ...
  const fetchData = async (): Promise<void> => {
    try {
      setLoading(true);
      const response = await requestPOST<IPaginationResponse<IDonViTinhDto[]>>('api/v1/nhacviecbaocaos/search', {
      ...
      try {
        switch (type) {
          ...
          case 'delete':
            ...
            const response = await requestDELETE<IResult<boolean>>(`api/v1/nhacviecbaocaos/${record.id}`);
