...
import { IDonViTinhDto, IResult } from '@/models';
...
const ModalItem = props => {
  ...
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IDonViTinhDto | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IDonViTinhDto>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IDonViTinhDto>>(`api/v1/nhacviecbaocaos/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IDonViTinhDto = {
        ...values,
        ...(id && { id }),
      };

      const response = id
        ? await requestPUT<IResult<string>>(`api/v1/nhacviecbaocaos/${id}`, formData)
        : await requestPOST<IResult<string>>(`api/v1/nhacviecbaocaos`, formData);
    ...
    >
      ...
      <Modal.Body>
        <Spin spinning={isLoading}>
          {!isLoading && (
            <Form<IDonViTinhDto> form={form} initialValues={{ suDung: true }} layout="vertical" autoComplete="off">
              <div className="row">
                ...
                <div className="col-xl-6 col-lg-6">
                  <FormItem label="Số ngày" name="soNgay" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <InputNumber placeholder="" min={0} max={1000} style={{ width: '100%' }} />
                  </FormItem>
                </div>
                <div className="col-xl-6 col-lg-6">
