...
import { IKyBaoCaoDto, IResult } from '@/models';
...
const ModalItem = props => {
  ...
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IKyBaoCaoDto | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IKyBaoCaoDto>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IKyBaoCaoDto>>(`api/v1/kybaocaos/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IKyBaoCaoDto = {
        ...values,
        ...(id && { id }),
      };

      const response = id
        ? await requestPUT<IResult<string>>(`api/v1/kybaocaos/${id}`, formData)
        : await requestPOST<IResult<string>>(`api/v1/kybaocaos`, formData);
    ...
    >
      ...
      <Modal.Body>
        <Spin spinning={isLoading}>
          {!isLoading && (
            <Form<IKyBaoCaoDto> form={form} initialValues={{ suDung: true }} layout="vertical" autoComplete="off">
              <div className="row">
