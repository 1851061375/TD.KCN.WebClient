...
import { IKyBaoCaoDto, IPaginationResponse, IResult } from '@/models';
...
const DataTable: React.FC<LoaiBaoCaosTableProps> = ({ searchData }) => {
  ...
  const [dataTable, setDataTable] = useState<IKyBaoCaoDto[]>([]);
  ...
  const handleAction = useCallback(
    async (type: string, record: IKyBaoCaoDto): Promise<void> => {
      try {
        switch (type) {
          case 'chi-tiet':
          ...
          case 'delete':
            ...
            const response = await requestDELETE<IResult<boolean>>(`api/v1/kybaocaos/${record.id}`);
  ...
  const columns: TableProps<IKyBaoCaoDto>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
