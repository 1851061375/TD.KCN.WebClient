...
const ModalItem = props => {
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IDonViTinhDto>>(`api/v1/donvitinhs/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`api/v1/donvitinhs/${id}`, formData)
        : await requestPOST<IResult<string>>(`api/v1/donvitinhs`, formData);
