...
const ModalItem = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadding(true);
      const res = await requestGET(`api/v1/loaicocautochucs/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT_NEW(`api/v1/loaicocautochucs/${id}`, formData) : await requestPOST_NEW(`api/v1/loaicocautochucs`, formData);
            ...
            <Form form={form} layout='vertical' /* initialValues={initData} */ autoComplete='off'>
              <div className='row'>
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Loại cơ cấu' name='tieuDe' rules={[{required: true, message: 'Không được để trống!'}]}>
                    <Input
                      placeholder=''
                      onChange={(e) => {
