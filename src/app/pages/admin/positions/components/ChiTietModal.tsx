...
import TDEditor from '@/app/components/tdeditor';
...
const ModalItem = props => {
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IPosition>>(`api/v1/positions/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`api/v1/positions/${id}`, formData)
        : await requestPOST<IResult<string>>(`api/v1/positions`, formData);
    ...
    >
      ...
      <Modal.Body>
        <Spin spinning={isLoading}>
          {!isLoading && (
            <Form<IPosition> form={form} layout="vertical" /* initialValues={initData} */ autoComplete="off">
              <div className="row">
